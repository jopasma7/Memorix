...
function getActiveDbPath() {
    ...
    return path.join(app.getPath('userData'), 'cementerio.db');
}
...
function createActivationWindow(reason) {
    ...
    activationWindow.webContents.once('did-finish-load', () => {
        const iconUrl = 'file://' + path.join(__dirname, 'assets', 'icon.png').replace(/\\/g, '/');
        activationWindow.webContents.send('activation-reason', reason);
        activationWindow.webContents.send('activation-icon', iconUrl);
    });
...
app.whenReady().then(async () => {
    ...
    const defaultDb = path.join(app.getPath('userData'), 'cementerio.db');
    const reg = loadDbRegistry();
...
ipcMain.handle('open-manual',        (_e, file)      => {
    const { shell } = require('electron');
    const manualPath = path.join(process.resourcesPath, file);
    const devPath    = path.join(__dirname, 'docs', file);
    shell.openPath(require('fs').existsSync(manualPath) ? manualPath : devPath);
});
...
// Etiquetas
ipcMain.handle('etiquetas-seed', async (_e, lang) => {
    try { await dbManager.seedDefaultEtiquetas(lang); return { ok: true }; }
    catch (e) { return { error: e.message }; }
});
ipcMain.handle('sample-data-seed', async (_e, lang) => {
    try { await dbManager.seedSampleData(lang); return { ok: true }; }
    catch (e) { return { error: e.message }; }
});
